...
// 판을 깨면 나오는 축하음(16.4): 여섯 가지 중 바로 앞과 다른 것을 골라 매번 다르게. 모두 장 으뜸음 5음계, 첫 박은 배경음 반박자에 맞춤.
//   0 오름 아르페지오  1 내려갔다 치솟기  2 세 화음 굴림(도미솔·라도미·솔시레→도)  3 두 옥타브 뛰기  4 종소리 물결  5 물음-대답
// 장을 다 깨면(big) 고른 것을 두 번 겹쳐 더 길고 크게.
const PENT2 = [1, 9/8, 5/4, 3/2, 5/3, 2, 9/4, 5/2, 3, 10/3, 4];
let lastFan = -1;
function toneClear(big = false) {
  duck(); clearTimeout(duckT); duckT = setTimeout(() => { if (AC) MUS.gain.setTargetAtTime(MUS_VOL, AC.currentTime, .8); }, big ? 4200 : 2600);
  ...
  let j; do { j = Math.floor(Math.random() * 6); } while (j === lastFan); lastFan = j;
  const M = (d, t, v = .34, p = 0, dur = .5) => marimba(root * d, t, v, p, dur), Kb = (d, t, v = .34, p = 0, dur = 1) => kalimba(root * d, t, v, p, dur);
  const S = big ? 1.25 : 1;                                     // 장 완성은 조금 더 크게
  const seqs = [
    () => { [0, 1, 2, 3, 4, 5, 6, 7].forEach((d, i) => M(PENT2[d], t0 + i * step, .34 * S, i / 7 - .5)); Kb(4, t0 + 8 * step, .36 * S, 0, 1.6); M(1, t0 + 8 * step, .3 * S, 0, 1.2); },
    () => { [5, 4, 3, 2].forEach((d, i) => M(PENT2[d], t0 + i * step, .3 * S, .4 - i * .25)); [0, 2, 4, 7].forEach((d, i) => M(PENT2[d], t0 + (4 + i) * step * .7, .36 * S, i / 3 - .5)); Kb(4, t0 + 4 * step + 4 * step * .7, .38 * S, .2, 1.6); },
    () => { const ch3 = [[1, 5/4, 3/2], [5/3, 2, 5/2], [3/2, 15/8, 9/4]]; ch3.forEach((c, k) => c.forEach((d, i) => M(d, t0 + k * step * 2 + i * .04, .3 * S, i / 2 - .5, .6))); [2, 5/2, 3, 4].forEach((d, i) => M(d, t0 + 6 * step + i * .045, .34 * S, i / 3 - .5, 1)); Kb(4, t0 + 6 * step + .1, .3 * S, 0, 1.5); },
    () => { [[1, 2], [5/4, 5/2], [3/2, 3], [2, 4]].forEach(([a, b], i) => { M(a, t0 + i * step * 1.5, .34 * S, -.4, .5); Kb(b, t0 + i * step * 1.5 + step * .5, .3 * S, .4, .7); }); M(2, t0 + 6 * step, .32 * S, 0, 1.4); Kb(4, t0 + 6 * step, .3 * S, 0, 1.6); },
    () => { for (let i = 0; i < 10; i++) Kb(PENT2[(i * 3) % 6] * 2, t0 + i * step * .5, (.22 + i * .012) * S, Math.sin(i) * .5, .9); [1, 5/4, 3/2, 2].forEach((d, i) => M(d, t0 + 5 * step + i * .04, .34 * S, i / 3 - .5, 1.2)); },
    () => { [0, 2, 4].forEach((d, i) => M(PENT2[d], t0 + i * step, .32 * S, -.4)); [4, 3, 2].forEach((d, i) => M(PENT2[d] * 2, t0 + (3.5 + i) * step, .3 * S, .4)); [1, 5/4, 3/2, 2].forEach((d, i) => { M(d, t0 + 7 * step, .3 * S, i / 3 - .5, 1.2); }); Kb(4, t0 + 7 * step + .05, .34 * S, 0, 1.6); },
  ];
  seqs[j]();
  if (big) { setTimeout(() => { if (!AC) return; const t1 = AC.currentTime + .05; [1, 5/4, 3/2, 2, 5/2, 3].forEach((d, i) => M(d, t1 + i * .05, .3, i / 5 - .5, 1.6)); Kb(4, t1 + .3, .34, 0, 2); Kb(6, t1 + .45, .26, .3, 2); }, (9 * step + .6) * 1000); }
}
...
let lastJD = -1;
function samulClear(big = false) {
  if (!AC) return;
  if (FX_MODE === 'tone') { toneClear(big); return; }
...
// 햅틱(16.4): 안드로이드 크롬은 navigator.vibrate로 떨림, 아이폰 사파리는 지원하지 않아 조용히 넘어간다. 숫자 하나(ms) 또는 [떨림, 쉼, 떨림…] 무늬.
const HAPTIC = { tap: 20, peel: 30, pop: w => Math.min(90, 30 + w * 12), finale: [90, 40, 160], clear: [70, 50, 70, 50, 200], chapter: [80, 50, 80, 50, 120, 50, 300], woke: [150, 70, 150], fail: [120, 60, 120], button: 15, star: 45 };   // 16.5 「더 강하게」: 전부 두 배쯤
...
const CARDS = {
  ...
  taps3:   { title: '세 번', kicker: '새로운 규칙', art: ['T3'], text: '세 번 칠 수 있어요. 판을 세 무리로 나눠 보세요. 뒤로 가면 네 번 판도 나와요 — 위의 등불 개수가 남은 횟수예요.' },
...
async function tap(i) {
  ...
  if (L.l > 0) { peel(i, L); buzz(HAPTIC.peel); await sleep(300); }
  else {
    ...
    wave.forEach((p, idx) => {
      ...
      for (const t of tr) {
        ...
        if (t.sleeper) {
          ...
          setTimeout(() => { el.querySelector('.zz').textContent = '!'; el.classList.add('woke'); yawn(); buzz(HAPTIC.woke); }, DUR * t.reach);
          continue;
        }
...
async function finish(won, woke = false) {
  ...
  if (won) {
    ...
    samulClear(chapterDone && first); buzz(chapterDone && first ? HAPTIC.chapter : HAPTIC.clear);
...
document.addEventListener('click', e => { if (e.target.closest('button') && !e.target.closest('.snd')) click(); if (e.target.closest('button')) buzz(HAPTIC.button); }, true);
// ---------- title screen ----------
