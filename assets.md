...
**받음**(`art/source-confetti.png`): 12조각을 덩어리별로 잘라 `art/cf0~11.webp`(가장 긴 쪽 72px, 합 23KB). 조각 속은 완전 불투명, 가장자리만 부드럽게. 판을 깰 때 `rain()`이 64개를 화면 위에서 떨어뜨린다 — 세 축으로 돌며 좌우로 흔들리고, 결과 카드 위(z 41)에 내린다.

## 11. 불티 (등불이 터질 때 튀는 불꽃) — 받음(시험판 14.1)
...
**받음**: 꼬리 있는 불티 한 장(`art/source-spark.png`). 검정을 밝기 기준으로 투명하게 바꿔 `art/spark.webp`(166×256, 27KB)로 넣고, 터질 때 `sparks()`가 크기별로 3~5개(큰 터짐은 +3)를 위쪽 반원으로 튀겨 포물선으로 떨어뜨린다. 꼬리는 진행 방향 반대로 돌리고 `mix-blend-mode: screen`으로 빛만 얹는다.

**둥근 것도 받음**(`art/source-spark2.png` → `art/glow.webp` 256², 18KB): 터짐 한가운데의 빛 번짐(`.glow`)이 예전 CSS 원형 그라데이션 대신 이 그림을 **가림막**으로 써서 장 색으로 칠해진다 — 불규칙한 번짐과 자잘한 불티 점이 살고, 장마다 색이 맞는다.

둘째 장 프롬프트(참고): 같은 문장에서 "아래쪽으로 짧고 가느다란 빛 꼬리가 하나"를 "꼬리 없이 둥글게, 둘레에 아주 작은 불티 점 네댓 개"로 바꾼다. 넣는 법: 검은 배경 그림은 `mix-blend-mode: screen`으로 얹으면 검정이 사라지고 빛만 남는다. 터질 때 조각과 함께 불티 3~5개가 튀어 오른 뒤 떨어진다.
...
**받음**(`art/source-river.png`, 1792×597): 오른쪽 ✦ 170px을 잘라내고 양끝 160px을 서로 섞어 이어붙여도 무늬가 맞게 만든 뒤 1400px 폭 `art/river.webp`(55KB). `.water` 배경으로 깔고 90초에 한 폭씩 왼쪽으로 흐른다. **16.2**: 「게임 화면과 강물이 안 맞는다」 → 원본의 주황 반사가 너무 밝고 채도가 높아 위 화면(먹빛·실루엣)과 따로 놀았다. 채도 55%, 밝은 부분 눌림, 전체 80%로 낮춰 다시 저장하고, 위쪽에 먹빛 그라데이션을 진하게 덧씌워 기슭과 이어지게, 장 색 빛을 아주 옅게 섞었다. 등불 반사(box-reflect)는 그대로 위에 얹힌다.
...
## 국악기 효과음 (시험판 11·13~15에 씀, 16부터 마림바·칼림바 합성음으로 바뀜)

**16.7: 공개 저장소에서 `sfx/gugak/`를 뺐다** — 녹음의 출처 표시가 필요한지 확인하지 못해서. 게임은 `FX_MODE='tone'`이라 영향이 없고, `loadGugak()`도 samul 모드일 때만 파일을 부른다. 원본과 잘라 낸 파일은 Claude 작업 공간과 사용자 다운로드 폴더에만 있다. 다시 쓰려면 출처를 확인해 README에 적은 뒤 올릴 것.
